...
export const load: LayoutServerLoad = async ({ locals, url, route, params }) => {
	...
	if(auth.client?.connected == false || auth.client?.connected == null) {
		return { protocol, domain, client_id, page, profile, access_token, wsurl, origin, collectionname: "entities", entities: [], workspaces: [], item: null, id: "", settings: {}, total_count, webcommit, webversion };
	}
	try {
		...
		return { protocol, domain, client_id, page, profile, access_token, wsurl, origin, collectionname, entities, workspaces, id, settings, total_count, webcommit, webversion };
	} catch (error) {
		console.error(error);
		return { protocol, domain, client_id, page, profile, access_token, wsurl, origin, collectionname: "entities", entities: [], workspaces: [], item: null, id: "", settings: {}, total_count, webcommit, webversion };
	}
